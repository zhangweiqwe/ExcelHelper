...
public class ExcelHelper {
	...
	private static File initMainFolder(){
		File mainFolder = new File(Environment.getExternalStorageDirectory()+"/"+Const.MAIN_FOLDER);
	...
	public static void createExcel(File file,List<ShoesSimple> shoesSimpleList) throws WriteException, IOException {
		...
		//预定义的一些字体和格式，同一个Excel中最好不要有太多格式   
		 ...
		 for(int i=0;i<shoesSimpleList.size();i++){
			 ...
			 label=new Label(column, row0-2, shoesSimple.getTitle(), headerFormat);   
			 sheet.addCell(label); 
			 
			
			 label=new Label(column, row0, "Id：", titleFormat);
			 sheet.addCell(label); 
			 label=new Label(column+1, row0, shoesSimple.getId()+"", detFormat);
			 sheet.addCell(label); 
			 row0++;
			 
			 label=new Label(column, row0, "时间：", titleFormat);   
			 sheet.addCell(label); 
			 label=new Label(column+1, row0, shoesSimple.getCreatTime(), detFormat);   
		     sheet.addCell(label); 
			 row0++;
